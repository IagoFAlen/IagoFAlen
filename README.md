<h1 align="left">Hi, I'm Iago 🇧🇷 — Developer and Lifelong Learner</h1>

<p>I work with systems that involve data processing, neural networks, recommendation systems, and secure back-end APIs. I'm especially interested in how things work under the hood — from C-based neural networks to fullstack apps connected to real-world data.</p>

---

### 🔍 Currently Exploring

- 🧠 Neural networks and supervised learning in C  
- 🛡️ Red Team / Pentesting  
- 🌐 TypeScript and scalable web architectures  
- 🧮 Algorithm design and optimization

---

### 🛠️ Tech Stack

**Languages:**  
![C](https://img.shields.io/badge/-C-111?style=flat&logo=c)
![C++](https://img.shields.io/badge/-C++-111?style=flat&logo=cpp)
![Python](https://img.shields.io/badge/-Python-111?style=flat&logo=python)
![JavaScript](https://img.shields.io/badge/-JavaScript-111?style=flat&logo=javascript)
![TypeScript](https://img.shields.io/badge/-TypeScript-111?style=flat&logo=typescript)
![SQL](https://img.shields.io/badge/-SQL-111?style=flat&logo=postgresql)

**Front-end:**  
![HTML5](https://img.shields.io/badge/-HTML5-111?style=flat&logo=html5)
![CSS3](https://img.shields.io/badge/-CSS3-111?style=flat&logo=css3)
![React](https://img.shields.io/badge/-React-111?style=flat&logo=react)

**Back-end / Tools:**  
![Node.js](https://img.shields.io/badge/-Node.js-111?style=flat&logo=node.js)
![Flask](https://img.shields.io/badge/-Flask-111?style=flat&logo=flask)
![Laravel](https://img.shields.io/badge/-Laravel-111?style=flat&logo=laravel)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-111?style=flat&logo=postgresql)
![RabbitMQ](https://img.shields.io/badge/-RabbitMQ-111?style=flat&logo=rabbitmq)
![Celery](https://img.shields.io/badge/-Celery-111?style=flat)

---

### 💼 Projects

- Adaptive neural network in C for skill classification and matchmaking  
- Product recommendation system using SBERT embeddings and category hierarchy  
- Design system built with TypeScript and responsive components  
- Mobile reading/tracking app developed as my B.Sc. final project (Node.js + PostgreSQL)

---

### 🎓 Education

- B.Sc. in Computer Science  
- Scientific research: Arduino + robotic movement in 2D space

---

### 📫 Contact

- 💬 Let’s build something!  
- 🔗 [LinkedIn](https://www.linkedin.com/in/iagofigueira)  
- 📧 iago.figueira@protonmail.com

---

<img src="https://i.pinimg.com/originals/f1/ed/a4/f1eda4768df8d8135c779772f2833e88.gif" width="100%">
